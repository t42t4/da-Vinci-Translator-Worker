...
・原文に「<@数字>」が含まれていない場合は、いかなる名前や呼称に対しても、太字などの装飾や@記号を勝手に付与してはならない。
・特に、【変換辞書】にある名前に類似した「〜ちゃん」「〜醬」などの呼び名が含まれる場合、それを辞書に基づいた形式に置換せず、**原文の呼称のまま（装飾なし）**で文脈に沿った自然な翻訳をせよ。（例：「ノエちゃん」→「Noe醬」）
